...
@interface M_setViewController ()<UITableViewDataSource,UITableViewDelegate>
{
...
-(void)initPage{
    ...
    if (topImageArr.count>0) {
        ...
        self.mainScorllView.onePage =  ^(NSInteger pageIndex)
        {
            ...
            labelText.text= topImageArr[pageIndex][@"title"];
        };
        ...
        pageControl=[[UIPageControl alloc]initWithFrame:CGRectMake(0*Main_Screen_Width*topImageArr.count, Main_Screen_Height/2-43, Main_Screen_Width, 5)];
...
-(void)handleGer:(int)index{
    ...
    if ([[topImageArr objectAtIndex:index]objectForKey:@"id"]!=nil) {
        ...
        detail.shareImage = [ImageSizeManager getSmallImageWithOldImage:[UIImage imageWithData:data]];//[UIImage imageWithData:data];
        [self.navigationController pushViewController:detail animated:YES];
    }
}

-(void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView{
    int x=scrollView.contentOffset.x/Main_Screen_Width;
...
-(void)getPage:(int)num
{
    ...
    NSString *str =[NSString stringWithFormat:@"http://api.mfeiji.com/v1/products?page=%d",pageFlag];
    ...
    [manager GET:str parameters:nil success:^(AFHTTPRequestOperation *operation, id responseObject) {
        ...
        [allArr addObjectsFromArray:[dic objectForKey:@"products"]];
